...
/* (mind) 
    ...
    vetor de booleano     | com a info se é folha ou nao
...
void solve() {
    ...
    // ordenação topologica com BFS?
    vector<ll> ordem;
    queue<ll> q;
    q.push(1); // começar com 1
    while (!q.empty()) {
        ll u = q.front();
        q.pop();
        ordem.pb(u);
    ...
    reverse(all(ordem)); // calc das folhas para a raiz
    // part parecida com a DFS
    for (ll u : ordem) { // vai começar das folhas
        sum[u] = peso[u]; 
        for (ll fi : adj[u]) {
            sum[u] += sum[fi];
        }
