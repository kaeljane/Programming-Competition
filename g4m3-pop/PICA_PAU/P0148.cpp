...
/* (mind) 
    ...
    lista com as qualidades do melhor competidor de cada cidade
...
void solve() {
    ...
    vector<ll> maxSuf(n+2, 0);

    f (i, 1, n+1) {
        maxPref[i] = max(maxPref[i-1], v[i-1]);
    }

    rf (i, n, 1) {
        maxSuf[i] = max(maxSuf[i+1], v[i-1]);
    }
    f (i, 0, q) {
        cin>>x;
        cout << maxPref[x-1] + maxSuf[x+1] << el;
    }
